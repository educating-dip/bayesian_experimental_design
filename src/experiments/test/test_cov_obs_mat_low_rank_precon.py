...
from scalable_linearised_laplace import (
        ...
        get_unet_batch_ensemble, get_fwAD_model, get_cov_obs_low_rank)
...
def coordinator(cfg : DictConfig) -> None:
    ...
    for i, data_sample in enumerate(islice(loader, cfg.num_images)):
        ...
        U, L, low_rank_cov_obs_mat_inv = get_cov_obs_low_rank(random_matrix_T, ray_trafos, filtbackproj.to(reconstructor.device),
            bayesianized_model, reconstructor.model, fwAD_be_model, fwAD_be_modules,
            log_noise_model_variance_obs, cfg.mrglik.impl.vec_batch_size, cfg.mrglik.preconditioner.reduced_rank_dim, cfg.mrglik.preconditioner.oversampling_param, use_fwAD_for_jvp=True)
        print('time in sec to approx cov_obs_mat {:.4f}'.format(time.perf_counter() - start))

        low_rank_cov_obs_mat = (U @ torch.diag(L) @ U.T) + torch.exp(log_noise_model_variance_obs) * torch.eye(U.shape[0],
            device=bayesianized_model.store_device)
        ...
        low_rank_cov_obs_mat_op = odl.operator.tensor_ops.MatrixOperator(low_rank_cov_obs_mat.cpu().numpy(),
            domain=batch_obs_domain, range=batch_obs_domain)

        print('opnorms')
        print('opnorm of cov_obs_mul_op', odl.operator.oputils.power_method_opnorm(cov_obs_mul_op, maxiter=100, rtol=1e-05, atol=1e-08))
        print('opnorm of low_rank_cov_obs_mat_op', odl.operator.oputils.power_method_opnorm(low_rank_cov_obs_mat_op, maxiter=100, rtol=1e-05, atol=1e-08))
        ...
        print('opnorm of low_rank_cov_obs_mat_op-cov_obs_mul_op', odl.operator.oputils.power_method_opnorm(low_rank_cov_obs_mat_op-cov_obs_mul_op, maxiter=100, rtol=1e-05, atol=1e-08))
        ...
        low_rank_cov_obs_mat_op_results = []
        for _ in range(1000):
            ...
            low_rank_cov_obs_mat_op_results.append(low_rank_cov_obs_mat_op(test_obs))
        ...
        print('mse of low_rank_cov_obs_mat_op(v) - cov_obs_mul_op(v)', np.mean(
                np.asarray(low_rank_cov_obs_mat_op_results) - np.asarray(cov_obs_mul_op_results))**2)
        ...
        if compare_with_assembled_jac_mat_mul:
            ...
            print('opnorm of cov_obs_mat.T-Kyy', odl.operator.oputils.power_method_opnorm(odl.operator.tensor_ops.MatrixOperator((cov_obs_mat.T-Kyy.detach()).cpu().numpy()), maxiter=100, rtol=1e-05, atol=1e-08))
            print('opnorm of low_rank_cov_obs_mat_op-Kyy', odl.operator.oputils.power_method_opnorm(odl.operator.tensor_ops.MatrixOperator((low_rank_cov_obs_mat-Kyy.detach()).cpu().numpy()), maxiter=100, rtol=1e-05, atol=1e-08))
            print('opnorm of low_rank_cov_obs_mat_op.T-Kyy', odl.operator.oputils.power_method_opnorm(odl.operator.tensor_ops.MatrixOperator((low_rank_cov_obs_mat.T-Kyy.detach()).cpu().numpy()), maxiter=100, rtol=1e-05, atol=1e-08))
            print('opnorm of low_rank_cov_obs_mat_op_inv-Kyy_inv', odl.operator.oputils.power_method_opnorm(odl.operator.tensor_ops.MatrixOperator((low_rank_cov_obs_mat_inv-torch.inverse(Kyy.detach())).cpu().numpy()), maxiter=100, rtol=1e-05, atol=1e-08))
